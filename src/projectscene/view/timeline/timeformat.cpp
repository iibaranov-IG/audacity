    ...
    if (tickType == TickType::MAJOR || tickType == TickType::MINOR) {
        ...
        if (intervalInfo.minor >= 3600.0) {
            ...
            if (intervalInfo.digits > 0) {
                const int wholeSeconds = static_cast<int>(floor(d));
                const int hours = wholeSeconds / 3600;
                const int minutes = (wholeSeconds / 60) % 60;
                const double seconds = d - (hours * 3600.0) - (minutes * 60.0);
                const QString secondsText = QString::number(seconds, 'f', intervalInfo.digits)
                                                .rightJustified(3 + intervalInfo.digits, QChar('0'));
